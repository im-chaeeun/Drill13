...
class Zombie:
    ...
    def __init__(self, x=None, y=None):
        ...
        self.tx, self.ty = 300, 300
        self.build_behavior_tree()

        self.patrol_locations = [
            (43, 274), (1118, 274), (1050, 494), (575, 804), (234, 991), (575, 804), (1050, 494)
        ]
        self.loc_no = 0
    ...
    def move_slightly_to(self, tx, ty, r):
        self.dir = math.atan2(ty - self.y, tx - self.x)
        if r:   self.speed = RUN_SPEED_PPS
        else:
    ...
    def move_to(self, r = 0.5):
        self.state = 'Walk'
        self.move_slightly_to(self.tx, self.ty, True)
    ...
    def move_to_boy(self, r=0.5):
        self.state = 'Walk'
        self.move_slightly_to(play_mode.boy.x, play_mode.boy.y, True)
        if self.distance_less_than(play_mode.boy.x, play_mode.boy.y, self.x, self.y, r):
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.RUNNING
    ...
    def avoid_boy(self, r=0.5):
        self.state = 'Walk'
        self.move_slightly_to(play_mode.boy.x, play_mode.boy.y, False)
        if self.distance_less_than(play_mode.boy.x, play_mode.boy.y, self.x, self.y, r):
            return BehaviorTree.SUCCESS
        else:
            return BehaviorTree.RUNNING
    ...
    def build_behavior_tree(self):
        a1 = Action('Set_target_location', self.set_target_location, 500, 50)
        ...
        # 소년에게 이동
        a4 = Action("소년에게 이동", self.move_to_boy)
        # 소년으로부터 도망감
        a6 = Action("소년에게서 도망", self.avoid_boy)

        c1 = Condition("소년이 근처에 있는지",self.is_boy_nearby, 7)
        ...
        root = Drill_SEL_chase_or_wander = Selector("추적 or 도망 or 배회", SEQ_move_boy,SEQ_avoid_boy, SEQ_wander)
